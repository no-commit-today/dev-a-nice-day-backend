...
class SummarizationPromptTest {

    @Test
    void CollectedContent_로부터_생성할_수_있다() {
        // given
        CollectedContent content = new CollectedContentBuilder()
                .title("컨텐츠 제목")
                .content("컨텐츠 내용")
                .build();

        // when
        SummarizationPrompt prompt = SummarizationPrompt.of(content);

        // then
        assertThat(prompt.getContent()).isEqualTo("""
                <목표>
