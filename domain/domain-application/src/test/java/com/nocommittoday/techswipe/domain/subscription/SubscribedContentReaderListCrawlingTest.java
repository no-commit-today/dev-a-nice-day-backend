...
class SubscribedContentReaderListCrawlingTest {
    ...
    void 컨텐츠_목록_리스트의_url을_크롤링한_후_url을_통해_컨텐츠를_크롤링한다() {
        // given
        ListCrawling listCrawling = new ListCrawling(
                "list-url",
                new Crawling(
                        CrawlingType.INDEX,
                        null,
        ...
        UrlListCrawlingIterator iterator = Mockito.mock(UrlListCrawlingIterator.class);
        ...
        DocumentCrawler documentCrawler = Mockito.mock(DocumentCrawler.class);
        given(documentConnector.connect("content-url-1")).willReturn(ClientResponse.success(documentCrawler));

        ContentCrawling contentCrawling = new ContentCrawling(
                new Crawling(
                        CrawlingType.INDEX,
                        null,
                        List.of(2)
        ...
        // when
        List<SubscribedContent> result = subscribedContentReaderListCrawling.getList(
    ...
    void date_이상의_컨텐츠를_가져온다() {
        ...
        DocumentCrawler documentCrawler1 = Mockito.mock(DocumentCrawler.class);
        given(documentConnector.connect("content-url-1")).willReturn(ClientResponse.success(documentCrawler1));

        DocumentCrawler documentCrawler2 = Mockito.mock(DocumentCrawler.class);
