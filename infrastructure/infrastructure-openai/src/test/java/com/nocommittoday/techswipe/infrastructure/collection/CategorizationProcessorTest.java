...
class CategorizationProcessorTest {
    ...
    void 내용을_형식에_맞게_반환한다() {
        ...
        // when
        final CategorizationResult result = categorizationProcessor.categorize(collectedContent);

        // then
        assertThat(result.success()).isTrue();
        assertThat(result.categories()).containsExactlyInAnyOrder(CollectionCategory.SW_ENGINEERING, CollectionCategory.SERVER);
    ...
    void 카테고리_수는_1개가_될_수_있다() {
        ...
        // then
        assertThat(result.success()).isTrue();
        assertThat(result.categories()).containsExactlyInAnyOrder(CollectionCategory.SW_ENGINEERING);
        assertThat(result.exception()).isNull();
    }
